    ...
    stage('AWS Deployment') {
      steps {
          withCredentials([
            ...
            sh 'git clone https://github.com/chinthanippurajesh22/mynode-app-terraform.git'
            sh '''
               cd node-app-terraform
               terraform init
               terraform apply -auto-approve -var access_key=${AWS_KEY} -var secret_key=${AWS_SECRET}
               git add terraform.tfstate
               git -c user.name="Rajesh" -c user.email="chinthaXXXX@gmail.com" commit -m "terraform state update from Jenkins"
               git push https://${REPO_USER}:${REPO_PASS}@github.com/chinthanippurajesh22/mynode-app-terraform.git master
            '''
        }
      }
    }
